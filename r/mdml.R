  ...
  mods <- lapply(
    ...
    function(mns) {
      ...
      mod <- ranger::ranger(y = y, x = x, probability = prob,
                            min.node.size = mns, 
                            num.threads = as.integer(Sys.getenv("RANGER_XGB_CORES")), ...)
    }
  )
    ...
    if (model == "ranger") {
      ...
      if (tune_params & is.null(mns)) {
        ...
        mod <- ranger::ranger(y = y, x = x, 
                              as.integer(Sys.getenv("RANGER_XGB_CORES")), ...)
      }
...
pred <- function(mod, x, ...) {
  ...
  if (is.numeric(mod)) {
    ...
    if (mod$treetype == "Probability estimation") {
      ...
      preds <- predict(mod, x, num.threads = as.integer(Sys.getenv("RANGER_XGB_CORES")),
                       ...)$predictions[, attr(mod, "targ_col")]
      return(preds)
    } else {
      
      return(predict(mod, x, num.threads = as.integer(Sys.getenv("RANGER_XGB_CORES")), 
                     ...)$predictions)
    }
...
ia_vals_mdml <- function(data, X, Z, W, Y, x0, x1, method, model, rep, nboot,
                         tune_params, params) {
  ...
  # SE x TE
  sete1 <- msd_two(yx1, "id0", -y, "id1", "xse_x1x0", boots)$value
  sete2 <- msd_two(yx0, "id1", -y, "id0", "xse_x0x1", boots)$value
  sete <- sete1 - sete2
  
  # DE x IE
  deie1 <- msd_two(yx1wx0, "id0", -y, "id0", "ctfse_x1x0", boots)$value
  deie2 <- msd_two(yx1, "id0", -yx0wx1, "id0", "xde_x1x0", boots)$value
  deie <- deie1 - deie2
  
  # DE x IE (in x1 group)
  deie1_x1 <- msd_two(yx1wx0, "id1", -yx0, "id1", "xde_x0x1_x1", boots)$value
  deie2_x1 <- msd_two(y, "id1", -yx0wx1, "id1", "xde_x1x0_x1", boots)$value
  deie_x1 <- deie1_x1 - deie2_x1
  
  # SE x DE
  sede1 <- msd_two(yx1wx0, "id0", -y, "id0", "xde_x0x1", boots)$value
  sede2 <- msd_two(yx1wx0, "id1", -yx0, "id1", "xde_x0x1", boots)$value
  sede <- sede1 - sede2
  
  # SE x IE
  seie1 <- msd_two(yx0wx1, "id0", -y, "id0", "xde_x0x1", boots)$value
  seie2 <- msd_two(yx0wx1, "id1", -yx0, "id1", "xde_x0x1", boots)$value
  seie <- seie1 - seie2
  ...
  ret <- rbind(
    data.frame(value = sete, inner_boot = seq_len(nboot), ia = "TE x SE"),
    data.frame(value = deie, inner_boot = seq_len(nboot), ia = "DE x IE"),
    data.frame(value = sede, inner_boot = seq_len(nboot), ia = "DE x SE"),
    data.frame(value = seie, inner_boot = seq_len(nboot), ia = "IE x SE"),
    data.frame(value = dis, inner_boot = seq_len(nboot), ia = "DE x IE x SE")
  )
